...
@section('title', 'MCQ Dashboard')
@section('page-title', 'Dashboard')
@section('page-subtitle', $isAdmin ? 'Admin Control Panel' : 'Welcome back, ' . $user->name)
...
<div class="main-content-grid grid grid-cols-1 lg:grid-cols-2">
    ...
    <div class="content-card-body">
      <div class="quick-actions-grid grid grid-cols-2">
        ...
        <a href="{{ route('mcq.analytics') }}" wire:navigate class="quick-action-btn border-dashed border-orange-300 hover:border-orange-500 hover:bg-orange-50 transition-colors group block text-center">
          ...
          <a href="{{ route('admin.notes.index') }}" class="quick-action-btn border-dashed border-indigo-300 hover:border-indigo-500 hover:bg-indigo-50 transition-colors group block text-center">
            <i data-lucide="sticky-note" class="text-indigo-500 group-hover:text-indigo-700 mx-auto"></i>
            <p class="text-sm font-medium text-indigo-700 group-hover:text-indigo-800">Moderate Notes</p>
          </a>
      </div>
    </div>
  </div>
  ...
  @else
  {{-- Student: Notes Spotlight, Available Exams, Leaderboard Preview --}}
  @if($canViewNotesHub)
  <div class="bg-white  rounded-2xl border-2 border-gray-300 shadow-sm">
    <div class="p-6 border-b border-gray-200  flex items-center justify-between">
      <h3 class="text-lg font-semibold text-gray-800 ">Study Notes Spotlight</h3>
      <a href="{{ route('mcq.notes.index') }}" wire:navigate class="text-sm font-semibold text-blue-600 hover:text-blue-700">View all →</a>
    </div>
    <div class="p-6 space-y-4">
      @forelse($notesPreview as $note)
        <article class="border border-gray-200 rounded-lg p-4 bg-gray-50">
          <div class="flex items-center justify-between gap-3">
            <span class="text-xs font-semibold text-gray-600 uppercase">{{ $note->question->mcqSet->title ?? 'MCQ Set' }}</span>
            <span class="text-xs text-gray-500">{{ $note->updated_at->diffForHumans() }}</span>
          </div>
          @if($note->title)
            <h4 class="text-sm font-semibold text-gray-900 mt-2">{{ $note->title }}</h4>
          @endif
          <p class="text-xs text-gray-600 mt-2 line-clamp-3">{{ Str::limit(strip_tags($note->content), 160) }}</p>
        </article>
      @empty
        <p class="text-sm text-gray-500">Capture your first note from the Read MCQ page to see it featured here.</p>
      @endforelse
    </div>
  </div>
  @endif
