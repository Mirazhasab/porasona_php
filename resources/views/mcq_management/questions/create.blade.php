...
@section('page-subtitle', 'Add a new question to ' . $mcqSet->title)

@section('content')
<div class="container mx-auto max-w-3xl">
    <div class="bg-white rounded-xl shadow-md p-8">
        <h2 class="text-2xl font-bold text-gray-800 mb-2">Add New Question</h2>
        <p class="text-gray-600 mb-6">MCQ Set: <strong>{{ $mcqSet->title }}</strong></p>
        
        <form id="questionForm" action="{{ route('mcq_questions.store') }}" method="POST">
            @php($nonce = bin2hex(random_bytes(16)))
            <input type="hidden" name="_nonce" value="{{ $nonce }}">
            @csrf
            <input type="hidden" name="mcq_set_id" value="{{ $mcqSet->id }}">
            ...
            <div class="mb-6">
                ...
                <select name="correct_ans" 
                    ...
                    <option value="1" {{ old('correct_ans') == '1' ? 'selected' : '' }}>A (First answer)</option>
                    <option value="2" {{ old('correct_ans') == '2' ? 'selected' : '' }}>B (Second answer)</option>
                    <option value="3" {{ old('correct_ans') == '3' ? 'selected' : '' }}>C (Third answer)</option>
                    <option value="4" {{ old('correct_ans') == '4' ? 'selected' : '' }}>D (Fourth answer)</option>
                </select>
                @error('correct_ans')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-6">
                ...
                <textarea name="notes" 
                          ...
                          rows="3"
                          class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent"
                          placeholder="Add explanation or hints for this question...">{{ old('notes') }}</textarea>
                @error('notes')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex gap-4">
                ...
                <a href="{{ route('mcq_sets.show', $mcqSet) }}" wire:navigate class="flex-1 bg-gray-300 hover:bg-gray-400 text-gray-800 px-6 py-3 rounded-lg transition-colors font-medium text-center">
                    <svg class="w-4 h-4 mr-2 inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24">
