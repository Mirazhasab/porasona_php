...
<div class="max-w-7xl mx-auto p-4">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 mb-6 p-4">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <form method="GET" action="" class="w-full lg:w-auto flex items-center gap-2">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search exams..." class="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-400 focus:border-blue-400 text-sm w-full lg:w-56" oninput="debouncedAjaxSearch(this)" autocomplete="off" />
                <script>
                let searchTimeout;
                function debouncedAjaxSearch(input) {
                    clearTimeout(searchTimeout);
                    searchTimeout = setTimeout(() => {
                        const keyword = input.value;
                        fetch(`{{ url('exams') }}?search=${encodeURIComponent(keyword)}&ajax=1`)
                            .then(res => res.text())
                            .then(html => {
                                const parser = new DOMParser();
                                const doc = parser.parseFromString(html, 'text/html');
                                const newList = doc.getElementById('exam-list-container');
                                if (newList) {
                                    document.getElementById('exam-list-container').innerHTML = newList.innerHTML;
                                }
                            });
                    }, 400);
                }
                        ...
                        @foreach($availableExams as $exam)
                            <div class="bg-white border border-gray-200 rounded-2xl shadow-sm hover:shadow-md transition-all duration-200 hover:-translate-y-1">
                                <div class="p-4">
                                    ...
                                    @if($exam->exam_name)
                                        <p class="text-gray-500 text-sm mb-3 flex items-center gap-3">
                                            <span class="flex items-center gap-1">
                                                <i class="fas fa-graduation-cap"></i>
                                                {{ $exam->exam_name }}
                                            </span>
                                            @if($exam->category)
                                            <span class="flex items-center gap-1 px-2 py-1 bg-blue-50 border border-blue-200 rounded-full text-xs font-medium">
                                                <i class="fas fa-tags text-blue-400"></i>
                                                <span class="text-blue-700">{{ $exam->category }}</span>
                                            </span>
                                            @else
                                            <span class="flex items-center gap-1 px-2 py-1 bg-blue-50 border border-blue-200 rounded-full text-xs font-medium">
                                                <i class="fas fa-tags text-blue-400"></i>
                                                <span class="text-blue-700">No Category</span>
                                            @endif
                                        </p>
                                    @endif
